		...
		try {
			...
			if(_cargaSelectiva == 0){
				if(_usuario.getTipoUsuario().equals("superadministrador")){
					pstmt = _miConexion.prepareStatement("SELECT * FROM pool_objetos WHERE tipo_objeto = 1 "+addSQL);
				}else{
				...
				if(_usuario.getTipoUsuario().equals("superadministrador")){
					pstmt = _miConexion.prepareStatement("SELECT * FROM pool_objetos WHERE tipo_objeto = 1 AND id_pool_objetos NOT IN(SELECT id_pool_objetos FROM instancia_objetos) "+addSQL);
				}else{
