 ...
 * Bajo la tutoráa del Prof. Francisco Mirabal.
...
public class EstructuraBase extends Objeto{

	public EstructuraBase(){
		super();
	}

	public EstructuraBase(Usuario _usuario, Connection _miConexion){
		super(_usuario, _miConexion);
	}

	public EstructuraBase(Usuario _usuario, Connection _miConexion, String _encuesta){
		super(_usuario, _miConexion, _encuesta);
		this.ingresaABd();
	}

	public EstructuraBase(Usuario _usuario, Connection _miConexion, int _idEncuesta){
		super(_usuario, _miConexion, _idEncuesta);
		this.recargaDeBd();
	}
	...
	private void recargaDeBd(){
		ResultSet rs = null;
		try {
			PreparedStatement pstmt = this.getConexion().prepareStatement("SELECT * FROM pool_objetos WHERE id_pool_objetos = ? AND tipo_objeto = 0");
		...
		if(!(this.getObjeto()).equals("")){
			try {
				if (getCargadaDeBD()) {
					...
					pstmt.setInt(2, 0);  //0 = estructura base
					...
					pstmt.setInt(3, 0); //0 = estructura base
		...
		try {
			...
			if(_cargaSelectiva == 0){
				if(_usuario.getTipoUsuario().equals("superadministrador")){
					pstmt = _miConexion.prepareStatement("SELECT * FROM pool_objetos WHERE tipo_objeto = 0 "+addSQL);
				}else{
					pstmt = _miConexion.prepareStatement("SELECT * FROM pool_objetos WHERE creado_por = ? AND tipo_objeto = 0 "+addSQL);
					if(!_soloPropias){
						pstmt = _miConexion.prepareStatement("SELECT * FROM pool_objetos WHERE (creado_por = ? OR creado_por IN (SELECT id_administradores FROM administradores WHERE tipo_administrador = 'superadministrador')) AND tipo_objeto = 0 "+addSQL);
					}
					pstmt.setInt(1, _usuario.getUsuarioId());
				}
			}else if(_cargaSelectiva == 1){
				if(_usuario.getTipoUsuario().equals("superadministrador")){
					pstmt = _miConexion.prepareStatement("SELECT * FROM pool_objetos WHERE tipo_objeto = 0 AND id_pool_objetos NOT IN(SELECT id_pool_objetos FROM instancia_objetos) "+addSQL);
				}else{
					pstmt = _miConexion.prepareStatement("SELECT * FROM pool_objetos WHERE creado_por = ? AND tipo_objeto = 0 AND id_pool_objetos NOT IN(SELECT id_pool_objetos FROM instancia_objetos) "+addSQL);
					if(!_soloPropias){
						pstmt = _miConexion.prepareStatement("SELECT * FROM pool_objetos WHERE (creado_por = ? OR creado_por IN (SELECT id_administradores FROM administradores WHERE tipo_administrador = 'superadministrador')) AND tipo_objeto = 0 AND id_pool_objetos NOT IN(SELECT id_pool_objetos FROM instancia_objetos) "+addSQL);
					}
					pstmt.setInt(1, _usuario.getUsuarioId());
				}
			}else{
